...
namespace VirtueSky.DataStorage
{
    public class SecurityPlayerPrefs
    {
        ...
        static SecurityPlayerPrefs()
        {
            ...
            string randomSeedForKey = "5b6fcb4aaa0a42acae649eba45a506ed";

            // 길이 상관 없고, aes에 쓸 key 와 iv 를 만들 용도
            string randomSeedForValue = "2e327725789841b5bb5c706d6b2ad898";

            {
                Rfc2898DeriveBytes key = new Rfc2898DeriveBytes(randomSeedForKey, saltBytes, 1000);
                _saltForKey = Convert.ToBase64String(key.GetBytes(blockSize / 8));
            }
        ...
        {
            return PlayerPrefs.HasKey(MakeHash(key + _saltForKey));
        }
